-- -------------------------------------------------------------------------- --
-- point_2d_arrays.ads          Dependable Computing
-- -------------------------------------------------------------------------- --

-- This package builds up some array comprehension functions using integer
-- arrays. These are designed so that they can be readily treated as templates
-- for generic functions. In building these, I'm avoiding actually using
-- generics because they prove more slowly and because they seem not to play
-- nicely in a proof setting in any case.

package point_2d_arrays with SPARK_Mode is
   -- "Inputs" to the generic would include:
   ...
   -- Types
   --
   -- Make-believe these are generic parameters:

   -- The element type for our arrays.
   type element_type is new vector_2d;

   -- An array of element_type of unspecified Postive range.
   ...
   -- input array.
   --
   -- I've had trouble getting generics and proofs to play nice, so here is
   -- a copy-pastable way to approach this. Simply change this definition when
   -- you want to use the filter function.
   function to_filter(element: element_type) return Boolean is
     (element.x > 0.0);
   ...
   -- I've had trouble getting generics and proofs to play nice, so here is
   -- a copy-pastable way to approach this. Simply change this definition when
   -- you want to use the map function.
   function to_map(element: element_type) return element_type is
     (element_type'(element.x * (-1.0), element.y * (-1.0)));
   ...
   type result_element_type is new Float;
   ...
   Default_Result_Element: constant result_element_type := 0.0;
   ...
   -- a copy-pastable way to approach this. Simply change this definition when
   -- you want to use the map function.
   function to_transform(element: element_type) return result_element_type is
      (result_element_type(element.x));
