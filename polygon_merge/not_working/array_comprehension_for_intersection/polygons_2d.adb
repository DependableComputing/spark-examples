      ...
      for vertex_index in polygon.vertices'First .. polygon.vertices'Last loop
         ...
         if vertex_index = polygon.vertices'Last then
            edges((vertex_index-polygon.vertices'First)+1) :=
              segment_2d'(p1 => polygon.vertices(polygon.vertices'Last),
         ...
         pragma Loop_Invariant(
          (for all edge_ind in edges'First..(vertex_index-polygon.vertices'First)+1 =>
            (for some vert_ind in polygon.vertices'First..vertex_index =>
             (if vert_ind  =  polygon.vertices'Last then
                  edges(edge_ind).p1 = polygon.vertices(polygon.vertices'Last) and
                  edges(edge_ind).p2 = polygon.vertices(polygon.vertices'First)
              else
                  edges(edge_ind).p1 = polygon.vertices(vert_ind) and
                  edges(edge_ind).p2 = polygon.vertices(vert_ind+1)))));
